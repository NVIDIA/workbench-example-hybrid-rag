...
#HTML STRINGS
farming_tab_html = f"""
                    <p>{TITLE} provides accessible and tailored advice for optimizing crop yields, thus applying better calculated strategies for handling the effects of climate change, enhancing Jamaica’s food security</p>
                                 """

researching_tab_html = f"""
                    <p>{TITLE} enables easy retrieval of information across vast documents for faster research and knowledge-sharing.</p>
                                                     """

policymaking_tab_html = f"""
                        <p>{TITLE} offers data-driven insights to develop more effective agricultural policies and strategies for sustainability.</p>
                    """
...
def build_page(client: chat_client.ChatClient) -> gr.Blocks:
    ...
    with gr.Blocks(title=TITLE, theme=kui_theme, css=kui_styles + _LOCAL_CSS) as page:
        ...
        gr.Markdown(f""" 
        <h1> {TITLE} </h1>
        <p>Derive insights on environmental, climatic and agriculural factors and strategies to assist in the following</p>
        """)
                                ...
                                with gr.Tabs(selected=0) as nim_tabs:
                                    ...
                                    with gr.TabItem("Remote", id=0) as remote_microservice:
                                        remote_nim_msg = gr.Markdown(
                                            "<br />Enter the details below. Then start chatting!")
                                    ...
                                    with gr.TabItem("Local", id=1) as local_microservice:
                                        gr.Markdown(
                                            "<br />**Important**: For AI Workbench on DOCKER users only. Podman is unsupported!")
